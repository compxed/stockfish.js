...
void P_hm::write_indices(const std::array<Piece, SQUARE_NB>& oldPieces,
    ...
    // PieceSquareIndex entries are multiples of 64, while s and orient use
    // only the low six bits. Therefore no carry crosses bit 6, and
    // (s ^ orient) + psi[pc] == s ^ (psi[pc] + orient), allowing the
    // orientation to be folded into the per-piece lookup offset.
