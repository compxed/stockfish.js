...
void AccumulatorStack::evaluate_side(Color                     perspective,
    ...
    {
#ifndef USE_LITE_NET
        // The hybrid king-move update is only useful for the full feature
        // sets: in Lite mode a king move that stays on the same side of the
        // board is a regular incremental update.
...
usize AccumulatorStack::find_last_usable_accumulator(Color perspective) const noexcept {
    ...
    for (usize curr_idx = size - 1; curr_idx > 0; curr_idx--)
    {
        ...
        // The feature set requires a refresh when the king crosses the center
        // of the board (any king move in the full build).
